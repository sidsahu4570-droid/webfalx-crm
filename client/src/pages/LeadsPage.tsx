...
export const LeadsPage: React.FC = () => {
  ...
  }, [search, status, isPermanentInterestedFilter, priority, dueOnly, callerId, sortBy, page, userRole, toast, categoryId, cityIdsKey]);
  ...
  }, [search, status, isPermanentInterestedFilter, priority, dueOnly, callerId, categoryId, selectedCityIds, user]);
  ...
  const checkLeadMatchesFilters = useCallback((lead: Lead, currentFilters: typeof filtersRef.current): boolean => {
    ...
    // 4. Caller scoping
    if (currentFilters.userRole === 'caller') {
      const leadUserId = typeof lead.userId === 'object' && lead.userId ? (lead.userId as any)._id : lead.userId;
      if (leadUserId && currentFilters.userId && leadUserId.toString() !== currentFilters.userId.toString()) {
        return false;
      }
    } else if (currentFilters.callerId) {
      const leadUserId = typeof lead.userId === 'object' && lead.userId ? (lead.userId as any)._id : lead.userId;
  ...
  const handleLeadUpdated = useCallback((updatedLead: Lead) => {
    ...
    // Always update detailLead if this lead is currently open in modal
    setDetailLead((prev) => (prev?._id === updatedLead._id ? updatedLead : prev));
  ...
  }, [socket, handleLeadUpdated, fetchLeads]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <FilterDropdown
            ...
            setIsPermanentInterested={(val) => { setIsPermanentInterestedFilter(val); setPage(1); }}
            onReset={() => {
              setStatus('All');
      ...
      {loading ? (
        ...
        <LeadTable
          ...
          onBulkAssignSuccess={fetchLeads}
